...
下拉菜单还会显示当前太平洋时间，并提供本地时间与太平洋时间之间的双向换算工具。显示时区默认读取并持续跟随 macOS 或 Windows 的系统时区，也可以在“时间 → 显示时区”中手动选择一个固定时区。太平洋时区使用 `America/Los_Angeles`（Windows 使用对应系统时区），会自动处理夏令时（PDT）与标准时间（PST）。Tibo 消息里识别出的太平洋时间也会转换到这里选择的显示时区。
...
- 跟随系统或手动指定显示时区、本地时间与太平洋时间双向换算、全年使用热力图和 GitHub 更新检查；
- 每 60 秒自动刷新，以及手动刷新；
